...
class Logs extends Events {
    constructor() {
        ...
        winston.addColors(this.LOG_LEVEL.colors);
        ...
        for (const level of Object.keys(this.LOG_LEVEL.levels)) {
            const origin = this.logger[level].bind(this.logger);
            this[level] = (...args) => {
                ...
                this.emit(level, args[0].message);
        ...
        process.on('uncaughtExceptionMonitor', (error) => {
            this.critical(`Uncaught Exception Monitor: [${error.stack || error.message}]`);
        });

        process.on('uncaughtException', (error) => {
            this.critical(`Uncaught Exception: [${error.stack || error.message}]`);
        });

        process.on('unhandledRejection', (reason, promise) => {
            this.alert(`Unhandled Rejection: ${reason}`);
        });

        process.on('warning', (warning) => {
            this.warn(`Node Warning: [${warning.name} - ${warning.message}]`);
        });

        process.on('beforeExit', (code) => {
            this.info(`Process beforeExit with code ${code}`);
        });

        process.on('exit', (code) => {
            this.info(`Process exiting with code ${code}`);
        });
    }
